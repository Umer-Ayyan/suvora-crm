...
export default function CreateQuotation({ clients }: { clients: Client[] }) {
  ...
  function setItem(i: number, field: keyof LineItem, val: string) {
  ...
  const tax = afterDiscount * (Number(taxPct) / 100);
  const total = afterDiscount + tax;

  function reset() {
    setClientId(""); setClientName(""); setClientEmail(""); setClientCompany("");
    setValidUntil(""); setDiscountPct("0"); setTaxPct("0"); setNotes("");
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-2xl rounded-2xl flex flex-col max-h-[92vh]"
            ...
            <div className="flex items-center justify-between px-6 py-4 flex-shrink-0"
              ...
              <div>
                <p className="text-base font-semibold text-white">New Quotation</p>
                <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>Fill in client details and line items</p>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <select value={clientId} onChange={(e) => selectClient(e.target.value)}
                      className={inp + " cursor-pointer"} style={inpStyle} onFocus={fo} onBlur={fb}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input value={clientName} onChange={(e) => setClientName(e.target.value)}
                          placeholder="Full name" className={inp} style={inpStyle} onFocus={fo} onBlur={fb} />
                      </div>
                      <div>
                        ...
                        <input value={clientCompany} onChange={(e) => setClientCompany(e.target.value)}
                          placeholder="Company name" className={inp} style={inpStyle} onFocus={fo} onBlur={fb} />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="email" value={clientEmail} onChange={(e) => setClientEmail(e.target.value)}
                          placeholder="client@email.com" className={inp} style={inpStyle} onFocus={fo} onBlur={fb} />
                      </div>
                      <div>
                        <label className="block text-xs mb-1" style={{ color: "rgba(255,255,255,0.4)" }}>Valid Until</label>
                        <input type="date" value={validUntil} onChange={(e) => setValidUntil(e.target.value)}
                          className={inp} style={inpStyle} onFocus={fo} onBlur={fb} />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.08)" }}>
                    <div className="grid grid-cols-12 gap-0 px-3 py-2"
                    ...
                    {items.map((item, i) => (
                      <div key={i} className="grid grid-cols-12 gap-2 px-3 py-2 items-center group"
                        ...
                        <input value={item.quantity} onChange={(e) => setItem(i, "quantity", e.target.value)}
                          ...
                          style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}
                          onFocus={fo} onBlur={fb} />
                        <input value={item.unitPrice} onChange={(e) => setItem(i, "unitPrice", e.target.value)}
                        ...
                        <div className="col-span-2 flex items-center justify-end gap-1">
                          ...
                          {items.length > 1 && (
                            <button type="button" onClick={() => removeItem(i)}
                              className="w-5 h-5 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity ml-1"
                              style={{ color: "#f87171" }}>
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="rounded-xl p-4 space-y-2" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
                    ...
                    {Number(discountPct) > 0 && (
                      <div className="flex justify-between text-sm">
                        <span style={{ color: "rgba(255,255,255,0.5)" }}>Discount ({discountPct}%)</span>
                        <span style={{ color: "#fca5a5" }}>-${discount.toFixed(2)}</span>
                      </div>
                    )}
                    {Number(taxPct) > 0 && (
                      <div className="flex justify-between text-sm">
                        <span style={{ color: "rgba(255,255,255,0.5)" }}>Tax ({taxPct}%)</span>
                        <span style={{ color: "#fcd34d" }}>+${tax.toFixed(2)}</span>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3}
                      ...
                      className={inp + " resize-none"} style={inpStyle} onFocus={fo} onBlur={fb} />
                  </div>
                  <div>
